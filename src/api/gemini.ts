...
class GeminiService {
  ...
  constructor() {
    // Using the API key directly for demo purposes
    this.genAI = new GoogleGenerativeAI('AIzaSyCTZ8K_LViHPPgiY8vwyCIpOc6jCFw41v4');
    this.model = this.genAI.getGenerativeModel({ model: "gemini-2.0-flash-lite" });
  }

  async generateThinkingSteps(query: string, hasFiles = false): Promise<ThinkingStep[]> {
  ...
  async generateResponse(query: string, context = '', files: any[] = []) {
    const response = `# Research Analysis: ${query}
...
${files.length > 0 ? `\n## File Analysis\nAnalyzed ${files.length} uploaded document(s) which provided additional context and supporting evidence for the above conclusions.` : ''}`;
  ...
  async generateMindMap(query: string, files: any[] = []): Promise<MindMapData> {
    ...
    if (files.length > 0) {
      ...
      mindMap.edges.push({ source: 'center', target: 'files' });
      ...
      files.forEach((file: any, index: number) => {
        ...
        mindMap.edges.push({ source: 'files', target: `file_${index}` });
      });
    }
  ...
  async expandNode(nodeId: string, currentMindMap: MindMapData, query: string) {
    const expandedNodes = [
      {
        id: `${nodeId}_detail_1`,
